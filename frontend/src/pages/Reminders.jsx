...
export default function Reminders() {
  ...
  const save = async () => {
    ...
    } catch { setMsg({ type: 'error', text: 'Failed to save settings. Please try again.' }) }
  ...
  const sendTest = async () => {
    ...
    try {
      ...
      setMsg({ type: 'success', text: data.message || 'Reminder email sent successfully! 📧' })
    } catch (err) {
      // Friendly message instead of raw API error
      setMsg({ type: 'error', text: "Couldn't send the email right now. Please try again in a moment." })
    }
  ...
  return (
    <div className="p-4 md:p-6 max-w-xl page-enter">
      ...
      {/* Message */}
      {msg.text && (
        <div className={`rounded-xl px-4 py-3 mb-4 text-sm ${msg.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200'}`}>
          {msg.text}
        </div>
      )}
